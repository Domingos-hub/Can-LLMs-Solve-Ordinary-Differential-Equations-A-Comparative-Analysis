# -*- coding: utf-8 -*-
"""Can LLMs Solve ODEs? A Comparative Analysis of Five State-of-the-Art Models.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1phRYM2u954jWQ2nBFzeYoxYi4IwAqRDj
"""
...
Task 03:
Generate two (2) new problems analogous to the one you just solved. Each problem must be valid, self-explanatory, and adhere to the same specifications and constraints as the original.


=====================================================================================================================================================================================================================================================================================================================================================================
Here comes 3  examples:

[QUESTION TYPE-Exponential Growth: A population of protozoa grows at a constant relative rate of 0.44 per individual per day. On day zero, the population consists of five  protozoa.Calculate the population size after 8 days?]
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
**Solution 03: Generate two (2) new analogous problems**


=================================================================================================================================================================================================================================================================================
[QUESTION TYPE-Newton’s Law of Cooling: A body whose temperature is 20° is placed in a medium which is kept at a constant temperature of 60°. In 5 minute, the body's temperature has risen to 30°. How long will it take for the body's temperature to increase to 40°?]
...
=======================================================================================================================================================================================
[QUESTION TYPE-Electric Circuits-RC A 6μF capacitor is charged through a 30kΩ resistor from a 48V battery. Calculate how long will
...
CACHE_FILE = Path('math_ODEsdataset_qtypecsv_.txt_ler')
...
# Carrega o dataset
df = pd.read_csv('math_ODEsdataset_qtypecsv_.txt')
...
# Salva o CSV final
df.to_csv('math_ODEsdataset_qtypecsv_.txt_models', index=False)
print("✅ Processamento completo! Resultados salvos.")

model=pd.read_csv('math_ODEsdataset_qtypecsv_.txt_models')
model

model['openai/gpt-5.2_class'].iloc[1]

model['deepseek/deepseek-r1_class'].iloc[3]

model['anthropic/claude-haiku-4.5_class'].iloc[3]

model['google/gemini-2.5-pro_class'].iloc[3]

model['meta-llama/llama-3.3-70b-instruct_class'].iloc[0]
...
# Função para extrair o primeiro bloco de texto até \n\n, ** ou ##
def extract_first_block(text):
    if pd.isna(text):
        return None

    # Captura do início até o primeiro \n\n, ** ou ##
    pattern = r"^(.*?)(?:\n\n|\*\*|##)"
...
# Dataset
model = pd.read_csv("math_ODEsdataset_qtypecsv_.txt_models_final_results.csv")

# Criar novas colunas com os blocos extraídos
for col in model_cols:
    clean_col_name = col.split("/")[-1].replace("_class","") + "_first_block"
    model[clean_col_name] = model[col].apply(extract_first_block)

# Visualizar primeiras linhas
print(model.head().to_string(index=False))

# Salvar dataset atualizado
model.to_csv("math_ODEsdataset_qtypecsv_.txt_models_final_results_with_first_blocks.csv", index=False)

model=pd.read_csv('math_ODEsdataset_qtypecsv_.txt_models_final_results_with_first_blocks.csv')
model
...
def extract_python_code(text):
    ...
    Retorna lista de blocos de código limpos.
    """
    pattern = r'```(?:python|Python)\s*(.*?)\s*```'
    code_blocks = re.findall(pattern, str(text), re.DOTALL)
    return [block.strip() for block in code_blocks if block.strip()]

def execute_python_code(code):
    """
    Executa código Python e retorna:
    - Último valor numérico da memória, ou
    - Última saída numérica do print, ou
    - Mensagem de erro
    """
    local_vars = {}
    ...
    try:
        # Executa o código capturando stdout
        exec(code, {}, local_vars)
        output = redirected_output.getvalue()

        # Procura por valores numéricos na memória
        numeric_values = [
            v for v in local_vars.values()
            if isinstance(v, (int, float))
        ]

        # Procura por números na saída impressa
        output_numbers = re.findall(
            r'[-+]?\d*\.\d+|\d+',
            output
        )

        # Prioriza valores da memória, depois saída impressa
        if numeric_values:
            return numeric_values[-1]
        elif output_numbers:
            return float(output_numbers[-1])
        else:
...
def extract_final_result(text):
    """
    Extrai o resultado final do texto usando múltiplas estratégias:
    1. Tags <result>
    2. Último número no texto
    3. Padrões específicos de resposta
    """
    text = str(text)

    # Padrão 1: Conteúdo entre tags <result>
    pattern_result = r'<result>\s*([^<]+)\s*</result>'
    matches = re.findall(pattern_result, text, re.IGNORECASE)
    if matches:
    ...
    # Padrão 2: Último número no texto
    numbers = re.findall(
        r'[-+]?\d*\.\d+([eE][-+]?\d+)?|\d+',
        text
    )
    ...
    # Padrão 3: Resposta final após padrões comuns
    final_patterns = [
        r'[Rr]esposta\s*[:\-]\s*([-+]?\d*\.\d+|\d+)',
        r'[Ff]inal\s*[Rr]esult\s*[:\-]\s*([-+]?\d*\.\d+|\d+)'
    ]
...
input_path = 'math_ODEsdataset_qtypecsv_.txt_models'
output_path = 'math_ODEsdataset_qtypecsv_.txt_models_final_results.csv'

# Carrega os dados
model_results = pd.read_csv(input_path)

# FIX: Update model_columns to target the '_class' columns
model_columns = [col for col in model_results.columns if col.endswith('_class')]

# Processa cada coluna de modelo
for model_col in tqdm(model_columns, desc="Modelos"):
    results = []
    for text in tqdm(model_results[model_col], desc=model_col, leave=False):
        # Tarefa 1: Extrair código
        code_blocks = extract_python_code(text)

        if not code_blocks:
            # Tarefa 3: Extrair resultado do texto se não houver código
            result = extract_final_result(text)
            # Substitui NaN por "Erro"
            results.append("Erro" if pd.isna(result) else result)
            continue

        # Tarefa 2: Executar código
        final_result = np.nan
        ...
        if np.isnan(final_result):
            # Tarefa 3: Fallback para extração de texto
            final_result = extract_final_result(text)

        # Substitui NaN por "Erro"
        results.append("Erro" if pd.isna(final_result) else final_result)
...
model_final=pd.read_csv('math_ODEsdataset_qtypecsv_.txt_models_final_results.csv')
model_final

model_final['deepseek/deepseek-r1_class'].iloc[1]

import pandas as pd
import re

# Função para extrair o texto entre o primeiro \n\n e o primeiro **
def extract_middle_block(text):
    if pd.isna(text):
        return None

    # Regex: começa após \n\n e captura até o primeiro **
    pattern = r"\n\n(.*?)\*\*"
    match = re.search(pattern, str(text), re.DOTALL)
...
# Criar novas colunas com os blocos extraídos
for col in model_cols:
    clean_col_name = col.split("/")[-1].replace("_class","") + "_middle_block"
    model[clean_col_name] = model[col].apply(extract_middle_block)

# Visualizar primeiras linhas
print(model.head().to_string(index=False))

# Salvar dataset atualizado
model.to_csv("math_ODEsdataset_qtypecsv_.txt_models_final_results_with_blocks.csv", index=False)

model=pd.read_csv("math_ODEsdataset_qtypecsv_.txt_models_final_results_with_blocks.csv")
model

model['deepseek-r1_middle_block'].iloc[0]

model=pd.read_csv('math_ODEsdataset_qtypecsv_.txt_models_final_results_with_blocks.csv')
model

model['deepseek-r1_middle_block'].iloc[1]
...
# Caminho do arquivo
csv_path = 'math_ODEsdataset_qtypecsv_.txt_models_final_results.csv'
